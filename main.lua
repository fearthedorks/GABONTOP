...
-- Start empty, then add via addEmote
-- Table of Emotes
local Emotes = {
...
-- Utility: Convert readable date ("Aug 08, 2025") → Unix time
local months = {
...
addEmote("Belly Dancing", 119260531035442, 39, "Aug 08, 2025")
...
addEmote("Stylish Floating", 88937413073906, 39, "Aug 15, 2025")
...
addEmote("I WANNA RUN AWAY", 75607448080831, 50, "Aug 29, 2025")
...
-- UI Setup (copied from reference)
...
Frame.MouseLeave:Connect(function() EmoteName.Text = "Select an Emote" end)
...
local function createsort(order, text, sort)
    ...
    bt.SizeConstraint = Enum.SizeConstraint.RelativeXX
    bt.Size = UDim2.new(1, 0, 0.2, 0)
    bt.BackgroundColor3 = Color3.fromRGB(30, 30, 30)
    bt.LayoutOrder = order
    bt.TextColor3 = Color3.new(1, 1, 1)
    bt.Text = text
    bt.TextScaled = true
    bt.BorderSizePixel = 0
    Corner:Clone().Parent = bt
    bt.Parent = SortFrame
...
SearchBar:GetPropertyChangedSignal("Text"):Connect(function()
    ...
    for _, btn in pairs(Frame:GetChildren()) do
        if btn:IsA("GuiButton") then
            local nm = btn:GetAttribute("name"):lower()
            btn.Visible = nm:match(txt) or txt == ""
        end
    end
end)
...
local function PlayEmote(name, id)
    ...
    if not (Humanoid and Description) then return end
...
-- Oldest first = by lastupdated ASC
table.sort(Emotes, function(a, b) return a.lastupdated < b.lastupdated end)
